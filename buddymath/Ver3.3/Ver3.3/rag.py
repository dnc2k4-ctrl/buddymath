"""
rag.py – RAG Engine for MathBuddy  (Gemini Edition)
Thay đổi so với bản cũ:
  • Embedder được inject vào (mặc định: GeminiEmbedder, 768-dim)
  • Hỗ trợ brute-force cosine search khi FAISS không có
  • Thêm get_all_chunks() để synthesis endpoint dùng
  • Index path đặt trong data/ để tách biệt với code
"""
...
class Embedder(Protocol):
    """Bất kỳ class nào implement hai method này đều dùng được."""
    dim: int
...
def _default_embedder() -> Embedder:
    """Tạo JinaEmbedder (Jina AI free API), fallback về random nếu lỗi."""
    try:
        ...
        logger.warning(f"Không khởi tạo được JinaEmbedder: {exc}. Dùng fallback.")
...
class RAGEngine:
    """
    Quản lý embedding model và vector index cho MathBuddy.

    Usage
    -----
    engine = RAGEngine()
    engine.add_documents(chunks)
    results = engine.search("phương trình bậc hai", top_k=5)
    """
        ...
        if self.index_path.exists():
            try:
                ...
                if idx.d != self.embedding_dim:
                    logger.warning(
                        f"FAISS index dim={idx.d} ≠ embedder dim={self.embedding_dim}. "
                        "Tạo index mới."
                    )
    ...
    def add_documents(self, documents: list[Document]) -> None:
        """Embed và lưu danh sách Document chunks."""
        if not documents:
            return

        texts   = [doc.content for doc in documents]
        vectors = self._embed_docs(texts)
        ...
        self._save_index()
        logger.info(
            f"Thêm {len(documents)} docs. "
            f"Tổng: {len(self._documents)}."
        )
    ...
    def search(
        ...
        Hỗ trợ filter theo subject, topic, doc_type.
        """
    ...
    def get_all_chunks(
        ...
        """
        Trả về TẤT CẢ chunks của một topic (không giới hạn top-k).
        Dùng bởi synthesis endpoint để tổng hợp toàn bộ tài liệu.
        """
        return [
    ...
    def list_topics(self, subject: str) -> list[str]:
        return sorted(
            {doc.topic for doc in self._documents
             if doc.subject.lower() == subject.lower()}
        )

    def get_topic_content(self, subject: str, topic: str) -> dict:
        """Trả về theory + exercises cho một topic."""
        theory = [
            doc for doc in self._documents
            if doc.subject.lower() == subject.lower()
            and doc.topic.lower()  == topic.lower()
